class SenderNode extends LiteGraph.LGraphNode {
    ...
    constructor() {
        ...
        this.bufferNameWidget = this.addWidget("text", "Buffer Name", this.properties.bufferName, (value) => {
            if (!value.trim()) {
                ...
                this.statusWidget.value = "Buffer name cannot be empty";
                return;
            }
    ...
    detectDataType(data) {
        ...
        if (typeof data === "boolean") return "[Trigger]";
        if (typeof data === "number") return "[Number]";
        if (typeof data === "string") return "[String]";
        if (typeof data === "object" && data !== null) {
            if (data.hasOwnProperty("hue") && data.hasOwnProperty("saturation") && data.hasOwnProperty("brightness") &&
                typeof data.hue === "number" && typeof data.saturation === "number" && typeof data.brightness === "number") {
                return "[HSV]";
            }
        ...
        if (bufferName) {
            ...
            if (inputData !== undefined && inputData !== this.lastInputValue) {
                ...
                if (existingPrefix && existingPrefix !== this.detectDataType(inputData)) {
                    this.statusWidget.value = `Buffer ${bufferName} has different type`;
...
// === EXPOSE BUFFER GLOBALLY FOR HUD (Optional) ===
window.SenderNodeSharedBuffer = SenderNode.sharedBuffer;