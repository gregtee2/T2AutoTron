if (!LiteGraph.registered_node_types?.["Logic/LessThanNode"]) {
  class LessThanNode extends LiteGraph.LGraphNode {
    constructor() {
      super();
    ...
    onExecute() {
      ...
      if (this.properties.debug) {
        console.log(`LessThanNode - Value 1: ${value1}, Value 2: ${value2}, Output: ${output}`);
      }
    ...
    onDrawForeground(ctx) {
      ...
      const resultText = `Output: ${this.outputState}`;
      ctx.fillText(resultText, this.size[0] / 2, this.size[1] - 10);
    }

    onConnectionsChange(type, index, connected, link_info) {
      if (type === LiteGraph.OUTPUT && this.properties.debug) {
        console.log(`LessThanNode - Output ${connected ? "connected" : "disconnected"} at slot ${index}, link: ${link_info?.id}`);
      }
    }
  ...
  LiteGraph.registerNodeType("Logic/LessThanNode", LessThanNode);
}