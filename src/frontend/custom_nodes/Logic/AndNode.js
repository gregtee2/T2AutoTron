class AND_OR_Node extends LiteGraph.LGraphNode {
    ...
    loadState() {
        ...
        if (state) {
            ...
            this.resultState = false; // Reset resultState to force re-evaluation
            this.variables = parsed.variables || {};
            this.previousA = parsed.previousA || null;
            if (this.properties.debug) console.log(`[LogicNode] Loaded state: ${JSON.stringify(parsed)}, Reset resultState to false`);
        }
    }
    ...
    onExecute() {
        ...
        const allInputsReceived = this.properties.expectedTopics.length === 0 ||
            this.properties.expectedTopics.every(topic => this.inputBuffer[topic] !== undefined);

        // If inputs aren't ready, don't update the result yet
        if (!allInputsReceived) {
            if (this.properties.debug) console.log(`[LogicNode] Waiting for all inputs`);
            // Ensure outputs are set based on the last known resultState
            this.setOutputData(0, this.resultState);
            this.setOutputData(1, !this.resultState);
        ...
        // Proceed with evaluation once all inputs are available
        const values = [];
        for (let i = 0; i < this.properties.maxInputs; i++) {
            const topic = this.properties.expectedTopics[i] || `input${i + 1}`;
