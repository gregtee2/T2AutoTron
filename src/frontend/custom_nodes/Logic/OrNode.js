if (!LiteGraph.registered_node_types?.["Logic/OrNode"]) {
  class OrNode extends LiteGraph.LGraphNode {
    constructor() {
      super();
      this.title = "Or";
    ...
    addInputSlot() {
      if (this.properties.inputCount >= 8) {
        if (this.properties.debug) console.log(`OrNode - Maximum inputs (8) reached`);
        return;
      }
      ...
      if (this.properties.debug) console.log(`OrNode - Added input slot, total: ${this.properties.inputCount}`);
    }

    removeInputSlot() {
      if (this.properties.inputCount <= 2) {
        if (this.properties.debug) console.log(`OrNode - Minimum inputs (2) reached`);
        return;
      }
      this.properties.inputCount--;
      this.updateInputs();
      ...
      if (this.properties.debug) console.log(`OrNode - Removed input slot, total: ${this.properties.inputCount}`);
    }
    ...
    onExecute() {
      ...
      if (this.properties.debug) {
        ...
        console.log(`OrNode - ${inputsStr}, Output: ${output}`);
      }
    ...
    onConnectionsChange(type, index, connected, link_info) {
      if (type === LiteGraph.OUTPUT && this.properties.debug) {
        console.log(`OrNode - Output ${connected ? "connected" : "disconnected"} at slot ${index}, link: ${link_info?.id}`);
      }
    }
  ...
  LiteGraph.registerNodeType("Logic/OrNode", OrNode);
}