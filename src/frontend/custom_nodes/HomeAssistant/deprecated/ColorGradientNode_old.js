  ...
  class ColorGradientNode extends LiteGraph.LGraphNode {
    constructor() {
      ...
      this.properties = {
        ...
        reconnectInterval: 600000, // Default to 10 minutes (600,000 ms)
      };
    ...
    onDrawForeground(ctx) {
      try {
        ...
        if (this.isInRange) {
          ctx.fillText(`Position: ${(this.position * 100).toFixed(1)}%`, this.size[0] / 2, y);
          y += textHeight;
        }
    ...
    syncWidgetsToProperties() {
      try {
        ...
        widgets.forEach(widget => {
          if (widget.name === "Color Mode") this.properties.colorMode = widget.value;
          else if (widget.name === "Wedge") this.properties.predefinedWedge = widget.value;
          else if (widget.name === "Start Hue") this.properties.startHue = Math.round(widget.value);
          else if (widget.name === "Start Saturation") this.properties.startSaturation = Math.round(widget.value);
          else if (widget.name === "Start Brightness") this.properties.startBrightness = Math.round(widget.value);
          else if (widget.name === "End Hue") this.properties.endHue = Math.round(widget.value);
          else if (widget.name === "End Saturation") this.properties.endSaturation = Math.round(widget.value);
          else if (widget.name === "End Brightness") this.properties.endBrightness = Math.round(widget.value);
          else if (widget.name === "Range Mode") this.properties.rangeMode = widget.value;
          else if (widget.name === "Range Start") this.properties.startValue = Math.round(widget.value);
          else if (widget.name === "Range End") this.properties.endValue = Math.round(widget.value);
          else if (widget.name === "Start Time Hours") this.properties.startTimeHours = Math.round(widget.value);
          else if (widget.name === "Start Time Minutes") this.properties.startTimeMinutes = Math.round(widget.value);
          else if (widget.name === "Start Time Period") this.properties.startTimePeriod = widget.value;
          else if (widget.name === "End Time Hours") this.properties.endTimeHours = Math.round(widget.value);
          else if (widget.name === "End Time Minutes") this.properties.endTimeMinutes = Math.round(widget.value);
          else if (widget.name === "End Time Period") this.properties.endTimePeriod = widget.value;
          else if (widget.name === "Timer Duration") this.properties.timerDuration = Math.round(widget.value);
          else if (widget.name === "Timer Unit") this.properties.timerUnit = widget.value;
          else if (widget.name === "Time Steps") this.properties.timeSteps = Math.round(widget.value);
          else if (widget.name === "Gradient Y Position") this.properties.gradientBarYPosition = Math.round(widget.value);
          else if (widget.name === "Override Brightness") this.properties.useBrightnessOverride = widget.value;
          else if (widget.name === "Brightness") this.properties.brightnessOverride = Math.round(widget.value);
          else if (widget.name === "Debug") this.properties.debug = widget.value;
          else if (widget.name === "Enable Reconnect") this.properties.enableReconnect = widget.value;
          else if (widget.name === "Reconnect Interval (ms)") this.properties.reconnectInterval = Math.round(widget.value);
        });
        if (this.properties.debug && this._logCount++ % this._logLimit === 0) {
          console.log(
            `[ColorGradientNode] Synced widgets to properties: startValue=${this.properties.startValue}, endValue=${this.properties.endValue}, endHue=${this.properties.endHue}`
          );
        }
      } catch (e) {
        console.error(`[ColorGradientNode] Error in syncWidgetsToProperties: ${e.message}`);
      }
    }

    serialize() {
      try {
        this.syncWidgetsToProperties();
        const data = super.serialize();
        data.properties = { ...this.properties };
        data.timerStart = this.timerStart;
        data.lastUpdateTime = this.lastUpdateTime;
        data.currentStep = this.currentStep;
        data._lastCacheDate = this._lastCacheDate;
        data.lastInputValue = this.lastInputValue;
        data._lastTimeStep = this._lastTimeStep;
        if (this.properties.debug && this._logCount++ % this._logLimit === 0) {
          console.log(
            `[ColorGradientNode] Serializing: startValue=${this.properties.startValue}, ` +
            `endValue=${this.properties.endValue}, startBrightness=${this.properties.startBrightness}, ` +
            `endBrightness=${this.properties.endBrightness}, endHue=${this.properties.endHue}`
          );
        }
        return data;
      } catch (e) {
        console.error(`[ColorGradientNode] Error in serialize: ${e.message}`);
    ...
    configure(data) {
      try {
        ...
        if (data.properties) {
          this.properties = {
            colorMode: data.properties.colorMode || "custom",
            predefinedWedge: data.properties.predefinedWedge || "warm-to-cool",
            startHue: data.properties.startHue !== undefined ? Math.round(data.properties.startHue) : 0,
            startSaturation: data.properties.startSaturation !== undefined ? Math.round(data.properties.startSaturation) : 100,
            startBrightness: data.properties.startBrightness !== undefined ? Math.round(data.properties.startBrightness) : 100,
            endHue: data.properties.endHue !== undefined ? Math.round(data.properties.endHue) : 240,
            endSaturation: data.properties.endSaturation !== undefined ? Math.round(data.properties.endSaturation) : 80,
            endBrightness: data.properties.endBrightness !== undefined ? Math.round(data.properties.endBrightness) : 90,
            rangeMode: data.properties.rangeMode || "numerical",
            startValue: data.properties.startValue !== undefined ? Math.round(data.properties.startValue) : 20,
            endValue: data.properties.endValue !== undefined ? Math.round(data.properties.endValue) : 30,
            startTimeHours: data.properties.startTimeHours !== undefined ? Math.round(data.properties.startTimeHours) : 10,
            startTimeMinutes: data.properties.startTimeMinutes !== undefined ? Math.round(data.properties.startTimeMinutes) : 0,
            startTimePeriod: data.properties.startTimePeriod || "AM",
            endTimeHours: data.properties.endTimeHours !== undefined ? Math.round(data.properties.endTimeHours) : 2,
            endTimeMinutes: data.properties.endTimeMinutes !== undefined ? Math.round(data.properties.endTimeMinutes) : 0,
            endTimePeriod: data.properties.endTimePeriod || "PM",
            timerDuration: data.properties.timerDuration !== undefined ? Math.max(1, Math.round(data.properties.timerDuration)) : 1,
            timerUnit: data.properties.timerUnit || "hours",
            timeSteps: data.properties.timeSteps !== undefined ? Math.max(1, Math.round(data.properties.timeSteps)) : 60,
            gradientBarYPosition: data.properties.gradientBarYPosition !== undefined ? Math.round(data.properties.gradientBarYPosition) : 520,
            useBrightnessOverride: data.properties.useBrightnessOverride !== undefined ? data.properties.useBrightnessOverride : false,
            brightnessOverride: data.properties.brightnessOverride !== undefined ? Math.round(data.properties.brightnessOverride) : 254,
            debug: data.properties.debug !== undefined ? data.properties.debug : false,
            enableReconnect: data.properties.enableReconnect !== undefined ? data.properties.enableReconnect : false,
          ...
          if (this.properties.startValue < this.rangeMin) {
            this.properties.startValue = this.rangeMin;
          } else if (this.properties.startValue > this.rangeMax) {
            this.properties.startValue = this.rangeMax;
          }
          if (this.properties.endValue < this.rangeMin) {
            this.properties.endValue = this.rangeMin;
          } else if (this.properties.endValue > this.rangeMax) {
            this.properties.endValue = this.rangeMax;
          }

          this.properties.startHue = Math.max(0, Math.min(360, this.properties.startHue));
          this.properties.endHue = Math.max(0, Math.min(360, this.properties.endHue));
          this.properties.startSaturation = Math.max(0, Math.min(100, this.properties.startSaturation));
          this.properties.endSaturation = Math.max(0, Math.min(100, this.properties.endSaturation));
          this.properties.startBrightness = Math.max(0, Math.min(100, this.properties.startBrightness));
          this.properties.endBrightness = Math.max(0, Math.min(100, this.properties.endBrightness));
          this.properties.brightnessOverride = Math.max(0, Math.min(254, this.properties.brightnessOverride));

          if (this.properties.debug && this._logCount++ % this._logLimit === 0) {
            console.log(
              `[ColorGradientNode] Deserialized: startValue=${this.properties.startValue}, ` +
              `endValue=${this.properties.endValue}, startBrightness=${this.properties.startBrightness}, ` +
              `endBrightness=${this.properties.endBrightness}, endHue=${this.properties.endHue}`
            );
          }
          this.updateWidgets();
          this.setupReconnectTimer();
          // Schedule initial reconnect 3 seconds after graph load
          setTimeout(() => {
            this.performReconnect();
            if (this.properties.debug) {
              console.log(`[ColorGradientNode] Performed initial reconnect 3 seconds after graph load`);
            }
          }, 3000);
          this.setDirtyCanvas(true, true);
        }
        this.timerStart = data.timerStart || null;
        this.lastUpdateTime = data.lastUpdateTime || null;
        this.currentStep = data.currentStep || 0;
        this._lastCacheDate = data._lastCacheDate || null;
        this.lastInputValue = data.lastInputValue || null;
        this._lastTimeStep = data._lastTimeStep || null;
        this.updateColorSwatch();
        this.updateSize();
      } catch (e) {
        console.error(`[ColorGradientNode] Error in configure: ${e.message}`);
      }
    }
    ...
    updateWidgets() {
      try {
        ...
        const startValueSlider = widgets.find(w => w.name === "Range Start");
        if (startValueSlider && this.activeWidget !== "Range Start") {
          const intendedStartValue = this.properties.startValue;
          startValueSlider.options.min = this.rangeMin;
          startValueSlider.options.max = this.rangeMax;
          const clampedValue = Math.max(this.rangeMin, Math.min(this.rangeMax, Math.round(intendedStartValue)));
          startValueSlider.value = clampedValue;
          this.properties.startValue = clampedValue;
          if (this.properties.debug) {
            console.log(
              `[ColorGradientNode] Updated Range Start slider: intendedStartValue=${intendedStartValue}, ` +
              `min=${startValueSlider.options.min}, max=${startValueSlider.options.max}, value=${startValueSlider.value}`
            );
          }
        }

        const endValueSlider = widgets.find(w => w.name === "Range End");
        if (endValueSlider && this.activeWidget !== "Range End") {
          const intendedEndValue = this.properties.endValue;
          endValueSlider.options.min = this.rangeMin;
          endValueSlider.options.max = this.rangeMax;
          const clampedValue = Math.max(this.rangeMin, Math.min(this.rangeMax, Math.round(intendedEndValue)));
          endValueSlider.value = clampedValue;
          this.properties.endValue = clampedValue;
          if (this.properties.debug) {
            console.log(
              `[ColorGradientNode] Updated Range End slider: intendedEndValue=${intendedEndValue}, ` +
              `min=${endValueSlider.options.min}, max=${endValueSlider.options.max}, value=${endValueSlider.value}`
            );
          }
        }

        widgets.forEach(widget => {
          if (this.activeWidget === widget.name) return;
