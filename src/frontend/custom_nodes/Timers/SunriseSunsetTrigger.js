if (!LiteGraph.registered_node_types["Timers/sunrise_sunset_trigger"]) {
    ...
    class SunriseSunsetTrigger extends LiteGraph.LGraphNode {
        constructor() {
            ...
            this.size = [550, 1000]; // Increased height for new widget
            ...
            this.properties = {
                ...
                fixed_stop_ampm: "PM",
                fixed_stop_enabled: true, // Fixed typo: was fixed_stop_enabledjade
                latitude: null,
                longitude: null,
                city: "",
            ...
            this.socket = io('http://localhost:3000'); // Initialize Socket.IO for logging
            ...
            this.log = (message, level = 'info') => {
                if (this.properties.debug || level === 'error') {
                    console.log(`[SunriseSunsetTrigger] ${message}`);
                    this.socket.emit('log', { message: `[SunriseSunsetTrigger] ${message}`, level, timestamp: new Date() });
                }
            };

            // Fetch HA token from server
            this.fetchHaToken().then(() => {
                try {
                    this.fetchSunTimes();
                    this.startCountdown();
                    if (typeof this.checkAndEmitState === 'function') {
                        ...
                        this.log("checkAndEmitState is not a function. Check class definition.", 'error');
                        this.updateStatus("Error: checkAndEmitState method missing");
                    }
                } catch (error) {
                    this.log(`Error in constructor: ${error.message}`, 'error');
        ...
        async fetchHaToken() {
            try {
                const response = await fetch('http://localhost:3000/api/ha-token');
                const data = await response.json();
                if (data.success && data.token) {
                    this.properties.haToken = localStorage.getItem('haToken') || '';
                    if (this.haTokenWidget) this.haTokenWidget.value = this.properties.haToken ? '********' : '';
                    this.log("Fetched HA token from server");
                }
            } catch (error) {
                this.log(`Failed to fetch HA token: ${error.message}`, 'error');
            }
        }
        ...
        setupWidgets() {
            ...
            addSectionHeader("Home Assistant");
            this.haTokenWidget = this.addWidget("text", "HA Token", this.properties.haToken ? '********' : "", v => {
                this.properties.haToken = v;
                localStorage.setItem('haToken', v);
                this.fetchSunTimes(0);
                this.log("Updated HA token");
            }, { width: sliderWidth });
            ...
            this.debugWidget = this.addWidget("toggle", "Debug", this.properties.debug, v => {
                ...
                this.log(`Debug ${v ? "enabled" : "disabled"}`);
            }, { width: sliderWidth });
        }
        ...
        updateStatus(newStatus) {
            ...
            this.setDirtyCanvas(true);
            this.log(`Status updated: ${newStatus}`);
        }
        ...
        async fetchSunTimes(retries = 3, delay = 2000) {
            ...
            if (this.properties.haToken) {
                try {
                    const response = await fetch('http://localhost:3000/api/sun/times', {
                    ...
                    if (data.success) {
                        ...
                        // Fetch HA config for timezone and location
                        try {
                            ...
                            if (configData.success) {
                                this.properties.timezone = configData.timezone || this.properties.timezone;
                                this.properties.latitude = configData.latitude || this.properties.latitude;
                                this.properties.longitude = configData.longitude || this.properties.longitude;
                            }
                        } catch (configError) {
                            this.log(`Config fetch error: ${configError.message}`, 'error');
                        }
                        ...
                        this.updateStatus("Sun times fetched successfully via Home Assistant.");
                        this.updateFinalTimes();
                        this.scheduleSunEvents();
                        usedHa = true;
                        return;
                    ...
                    this.log(`HA fetch error: ${error.message}`);
                    if (retries > 0) {
                        this.updateStatus(`Retrying HA fetch (${4 - retries}/3)...`);
            ...
            // Fallback to window.api.fetchSunTimes
            if (!usedHa && window.api && window.api.fetchSunTimes) {
                try {
                    this.log("Falling back to window.api.fetchSunTimes...");
                    ...
                    this.updateStatus("Sun times fetched successfully via fallback API.");
                    this.updateFinalTimes();
                    this.scheduleSunEvents();
                    return;
                } catch (error) {
                    this.log(`Fallback API error: ${error.message}`);
                }
            }

            // Ultimate fallback to default coordinates
            this.log("Using default coordinates after fetch failure.");
        ...
        computeNextEvents() {
            ...
            this.log(`Next On: ${this.properties.next_on_date?.toISOString()}, Next Off: ${this.properties.next_off_date?.toISOString()}`);
        }
            ...
            if ((this.properties.on_enabled || this.properties.fixed_on_enabled) && nextOn) {
                ...
                if (delayOn > -1000) {
                    this.onTimeoutId = setTimeout(() => {
                        ...
                        this.log(`On event triggered at ${new Date()}`);
                        this.scheduleSunEvents();
                    }, Math.max(delayOn, 0));
                }
            } else {
            ...
            if ((this.properties.off_enabled || this.properties.fixed_stop_enabled) && nextOff) {
                ...
                if (delayOff > -1000) {
                    this.offTimeoutId = setTimeout(() => {
                        ...
                        this.log(`Off event triggered at ${new Date()}`);
                        this.scheduleSunEvents();
                    }, Math.max(delayOff, 0));
                }
            } else {
        ...
        triggerOutput(state) {
            ...
            this.setDirtyCanvas(true);
            this.log(`Triggered state: ${state} at ${new Date()}`);
        }
        ...
        checkAndEmitState() {
            ...
            this.log(`State checked: ${this.currentState} at ${now}`);
        }
        ...
        isCurrentTimeWithinRange(now) {
            ...
            if (this.properties.fixed_stop_enabled && todayOff && now >= todayOff) {
                this.log(`Fixed stop time reached (${now.toFormat("hh:mm a")}), forcing Off`);
                return false;
            }
        ...
        onExecute() {
            ...
            this.log(`[onExecute] Output: ${this.currentState} at ${now}`);
        }
    ...
    // Ensure EventScheduler is defined
    const EventScheduler = window.EventScheduler || {
        registerEvent: (id, event) => console.log(`Registered event ${id}: ${event.description}`),
        unregisterEvent: (id) => console.log(`Unregistered event ${id}`)
    };
