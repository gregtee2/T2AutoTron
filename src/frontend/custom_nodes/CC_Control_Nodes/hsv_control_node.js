class HSVControlNode extends LiteGraph.LGraphNode {
    ...
    updateColorSwatch() {
        ...
        if (this.graph && this.graph.canvas) this.setDirtyCanvas(true);
    }
    // EXACT SAME COLOR SCIENCE AS AllInOneColorNode
    hsvToRgb(h, s, v) {
        const key = `${h}-${s}-${v}`;
    ...
    onExecute() {
        ...
        if (inputHSV) {
            // Support both HSV and RGB input
            if (inputHSV.r !== undefined || inputHSV.red !== undefined || Array.isArray(inputHSV)) {
                const rgb = Array.isArray(inputHSV)
                    ? inputHSV
                    : [inputHSV.r ?? inputHSV.red ?? 0, inputHSV.g ?? inputHSV.green ?? 0, inputHSV.b ?? inputHSV.blue ?? 0];
                const { hue, sat } = this.rgbToHsv(rgb[0], rgb[1], rgb[2]);
                this.properties.hueShift = Math.round(hue * 360);
                this.properties.saturation = Math.round(sat * 100);
                this.properties.brightness = Math.round(rgb[2] ?? 128);
            } else {
                this.properties.hueShift = Math.round((inputHSV.hue ?? 0) * 360);
                this.properties.saturation = Math.round((inputHSV.saturation ?? 0) * 100);
                this.properties.brightness = Math.round(inputHSV.brightness ?? 128);
            }
            this.sliders.hueShift.value = this.properties.hueShift;
            this.sliders.saturation.value = this.properties.saturation;
            this.sliders.brightness.value = this.properties.brightness;
