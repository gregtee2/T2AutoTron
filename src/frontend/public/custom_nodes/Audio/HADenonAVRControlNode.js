  ...
  class HADenonAVRControlNode extends LiteGraph.LGraphNode {
    ...
    log = (key, message, force = false) => {
      ...
      if (force || now - lastLog.time > 1000 || lastLog.message !== message) {
        //console.log(`HADenonAVRControlNode - ${message}`);
        ...
        const widgets = [
          ...
          },
          {
            ...
            callback: (v) => {
              ...
              this.updateAvrState({ volume: this.properties.volumeLevel });
            },
    ...
    getAvrOptions = () => {
      return this.deviceManagerReady && this.avrEntities.length
        ? this.avrEntities.map((e) => e.name || e.id)
        : ["No AVRs Found"];
    };
    ...
    onAvrSelected = async (value) => {
      ...
      const entity = this.avrEntities.find((e) => (e.name || e.id) === value);
      if (!entity) {
        this.updateStatus(`⚠️ AVR "${value}" not found.`);
        this.log("avrSelectedWarn", `No entity found for name: ${value}`, true);
        return;
      }
      this.properties.selectedAvrEntity = entity.id.replace('ha_', '');
      ...
      this.log("avrSelected", `Selected AVR "${value}" (${entity.id})`, true);
    };
    ...
    fetchAvrEntities = async () => {
      try {
        const response = await fetch(`${this.HA_API_URL}/api/media_players/ha`, {
        ...
        if (data.success && data.devices) {
          this.avrEntities = data.devices.filter(d => d.type === 'media_player');
          this.log("fetchAvrEntitiesDebug", `Filtered AVR entities: ${JSON.stringify(this.avrEntities, null, 2)}`, true);
          this.deviceManagerReady = true;
          this.updateStatus("✅ AVR entities fetched successfully.");
          this.log("fetchAvrEntities", "AVR entities fetched successfully", true);
          this.widgets.find((w) => w.name === "Select AVR").options.values = [
            "Select AVR",
            ...this.getAvrOptions()
          ];
      ...
      for (let attempt = 0; attempt < this.properties.maxRetries; attempt++) {
        try {
          const response = await fetch(
            `${this.HA_API_URL}/api/states/ha_${encodeURIComponent(this.properties.selectedAvrEntity)}`,
    ...
    updateSourceOptions = () => {
      ...
      const entity = this.avrEntities.find((e) => e.id === `ha_${this.properties.selectedAvrEntity}`);
    ...
    updateAvrState = async (update) => {
      ...
      const command = {
        entity_id: this.properties.selectedAvrEntity,
        service: update.volume !== undefined ? "media_player.volume_set" : update.power ? "media_player.turn_on" : "media_player.turn_off",
        data: update.volume !== undefined ? { volume_level: update.volume } : {},
        timestamp: Date.now()
      };
      this.commandQueue.push(command);
      this.log(
    ...
    updateNodeSize = () => {
      ...
      widgetsHeight += 15;
      this.size[1] = baseHeight + widgetsHeight + 45;
    ...
    onExecute = async () => {
      ...
      try {
        ...
        if (volumeInput !== undefined && Math.abs(volumeInput - this.avrState.volume) > 0.01) {
          this.lastVolumeInput = volumeInput;
          await this.updateAvrState({ volume: Math.max(0, Math.min(1, volumeInput)) });
          stateChanged = true;
        }
        if (sourceInput && sourceInput !== this.avrState.source) {
