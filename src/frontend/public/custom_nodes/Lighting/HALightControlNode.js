//working version pre filter change
if (!LiteGraph.registered_node_types?.["Lighting/HALightControlNode"]) {
  class HALightControlNode extends LiteGraph.LGraphNode {
    ...
    updateLightState = async (lightId, update, source = "application") => {
      ...
      if (!commandEnforceInput && now - this.lastCommandTimestamp < this.EXTERNAL_CHANGE_THRESHOLD && update.timestamp < this.lastCommandTimestamp) {
      ...
      this.log("updateLightState", `Constructed payload for ${lightName}: ${JSON.stringify(payload)}`, true);
      ...
      for (let attempt = 0; attempt < this.properties.maxRetries; attempt++) {
        try {
          ...
          const data = await response.json();
          this.log("updateLightStateResponse", `Response for ${lightName}: status=${response.status}, body=${JSON.stringify(data, null, 2)}`, true);
          ...
          if (data.success) {
            ...
            if (verifiedState.on === payload.on &&
                (!payload.brightness || Math.abs(verifiedState.brightness - payload.brightness) < 5) &&
                (!payload.hs_color || (Math.abs(verifiedState.hue - payload.hs_color[0]) < 1 && Math.abs(verifiedState.saturation - payload.hs_color[1]) < 1))) {
              this.perLightState[lightId] = {
                ...this.perLightState[lightId],
                on: payload.on,
              ...
              this.log("updateLightStateWarn", `State mismatch for ${lightName}: expected on=${payload.on}, bri=${payload.brightness}, got on=${verifiedState.on}, bri=${verifiedState.brightness}`, true);
            }
          } else {
      ...
      if (!success && this.socket.connected) {
        ...
        try {
          ...
          if (verifiedState.on === payload.on &&
            ...
            this.log("updateLightStateWarn", `Socket.IO state mismatch for ${lightName}: expected on=${payload.on}, bri=${payload.brightness}, got on=${verifiedState.on}, bri=${verifiedState.brightness}`, true);
          }
        } catch (error) {
    ...
    verifyLightStates = async (lights, update) => {
      ...
      for (const lightId of lights) {
        ...
        const matches = state.on === expectedOn &&
                       (!expectedBrightness || Math.abs(state.brightness - expectedBrightness) < 5) &&
                       (!expectedHue || Math.abs(state.hue - expectedHue) < 1) &&
                       (!expectedSaturation || Math.abs(state.saturation - expectedSaturation) < 1);
        if (!matches) {
          const lightName = this.properties.selectedLightNames[this.properties.selectedLightIds.indexOf(lightId)] || "Unknown";
    ...
    handleDeviceStateUpdate = async (data) => {
      ...
      this.commandQueue = this.commandQueue.filter((cmd) => !cmd.lights.includes(lightId));
    ...
    handleTrigger = async (trigger) => {
      ...
      try {
        const command = {
          ...
          update: { on: desiredState, enforceInput: this.properties.enforceInput, timestamp: Date.now() },
          timestamp: Date.now(),
        };
        this.commandQueue.push(command);
        this.log("handleTrigger", `Queued trigger command: on=${desiredState}, enforceInput=${command.update.enforceInput}, lights=${command.lights.length}`, true);
