if (!LiteGraph.registered_node_types || !LiteGraph.registered_node_types["Lighting/HueLightControlNode"]) {
    class HueLightControlNode extends LiteGraph.LGraphNode {
        ...
        async handleTrigger(trigger, force = false) {
            console.log(`handleTrigger: trigger=${trigger}, force=${force}, lastToggleInput=${this.lastToggleInput}, perLightState=${JSON.stringify(this.perLightState)}`);
            if (!this.properties.selectedLightIds.length) {
            ...
            if (force) {
                console.log(`handleTrigger: Forcing state fetch for all lights`);
                for (let retry = 0; retry < maxFetchRetries; retry++) {
                    ...
                    for (let i = 0; i < this.properties.selectedLightIds.length; i++) {
                        ...
                        if (!fetchSuccess) {
                            console.log(`handleTrigger: Fetch failed for light ${lightId} on retry ${retry + 1}`);
                            allFetched = false;
                            needsUpdate = true;
                            continue;
                        }
                    ...
                    if (allFetched) break;
                    console.log(`handleTrigger: Retrying state fetch, attempt ${retry + 2}`);
                    await new Promise(resolve => setTimeout(resolve, 500));
                }
            } else {
                for (let i = 0; i < this.properties.selectedLightIds.length; i++) {
                    ...
                    if (!lightId) continue;
                    const actualState = this.perLightState[lightId]?.on;
                    console.log(`handleTrigger: Light ${lightId} actualState=${actualState}, desiredState=${desiredState}`);
                    if (actualState !== desiredState) {
                        console.log(`handleTrigger: Mismatch detected for light ${lightId}`);
                        needsUpdate = true;
                    }
                }
            }
            ...
            if (!needsUpdate) {
                ...
                console.log(`HueLightControlNode - No update needed, state already matches: ${desiredState}`);
                return;
            }
            ...
            this.properties.selectedLightIds.forEach(lightId => {
                if (lightId) {
                    ...
                    console.log(`HueLightControlNode - Updated perLightState for ${lightId} to on=${desiredState}`);
                }
            });
            this.setDirtyCanvas(true);
            ...
            if (!this.isProcessingQueue) {
                console.log("HueLightControlNode - Processing queue for state change");
                this.processQueue();
            }
        }
        ...
        onExecute() {
            ...
            if (triggerInput !== undefined) {
                if (this.forceInitialTrigger && this.deviceManagerReady) {
                    setTimeout(() => {
                        console.log(`HueLightControlNode - Processing delayed initial trigger: ${triggerInput}, force=${this.forceInitialTrigger}`);
                    ...
                    console.log(`HueLightControlNode - Processing trigger: ${triggerInput}, force=false`);
