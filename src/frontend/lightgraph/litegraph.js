...
(function(global) {
        ...
        if (do_not_catch_errors) {
            for (var i = 0; i < num; i++) {
                for (var j = 0; j < limit; ++j) {
                    ...
                    // Skip nodes inside muted groups
    ...
    LGraph.prototype.isNodeMuted = function (node) {
        // Check if the node is contained in any muted BackdropGroup
        const mutedGroup = this._nodes.find(
            (n) =>
                n instanceof BackdropGroup &&
                n.properties.muted &&
                n.getContainedNodes().includes(node)
        );
